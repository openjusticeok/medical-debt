...
``` {r}
debt <- debt %>% 
  mutate(plaint_type = case_when(str_detect(iss_plaint, "HEALTH|CLINIC|MEDICAL|HOSPITAL|SURGICARE|INTEGRIS\\sAMBULATORY|CARDIOLOGY|TERENCE\\sC\\sTOFT\\sDDS|DENTIST|DENTAL|SURGERY|ORTHOPEDICS|INTEGRIS|ALLIED\\sPHYSICIANS\\sGROUP|SURGICAL|D\\.D\\.S\\.|ADVANCES\\sIN\\sDENTISTRY|MANORCARE|ANESTHESIOLOGY|OU\\sPHYSICIANS|PULMONARY|PHARMACEUTICALS|PHYSICAL\\sTHERAPY|RADIOLOGY| DDS|PEDIATRICS|MRI") ~ "MEDICAL",
                                 str_detect(iss_plaint, "BANK|FCU|CREDIT|LOAN|FINANCIAL|WORLD\\sACCEPTANCE|INVESTMENT|FINANCE|MONEY|MOTORS|CASH|B\\s&\\sF|PROFESSIONAL\\sINSURANCE\\sAGENCY\\sCORP|SOUTHWESTERN\\sBELL\\sYELLOW\\sPAGES|SINGLETON\\sENTERPRISES|B&F|B\\sAND\\sF|MANHATTAN\\sMANAGEMENT\\sCOMPANY|ROYAL\\sPRESTIGE|BONDS|TELEPHONE|DEPARTMENT\\sSTORES|AUTO|HOME|ADVANCE\\sAMERICA|STATE\\sOF\\sOKLAHOMA|ROCK\\sKNOLL\\sASSOCIATION|B&\\sF|ELECTRIC|RENT\\sA\\sCENTER|LIBERTY\\sBONDING|DEPARTMENT\\sOF\\sTRANSPORTATION|COMPREHENSIVE\\sTHERAPY\\sSERVICES|PHYSICAL\\sTHERAPY|COMPREHENSIVETHERAPY") ~ "OTHER",
                                 TRUE ~ "UNKNOWN"))
```
