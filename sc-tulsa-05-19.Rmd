---
title: "Small Claims: Tulsa County 05-19"
author:
...
sc <- ojo_query_disps("TULSA", "SC", 2005:2019)


```

This gives us over 447,711 rows of data. Each row in the table is a disposition; since there may be multiple dispositions per case, we should expect that the number of cases is somewhat lower.
...
```
There is no missing data for the issue description (iss_desc) column.
...
```{r}
mins <- ojo_query_mins("TULSA", "SC", 2005:2019, min_code = "A/")
...
#sc <- sc %>% 
  ...
  #slice(1)
``` 
That gives us 319,343 lawsuits filed. 
...
sc %>% 
  ...
  arrange(desc(n)) 
```
We see that over 284,899 cases are evictions. We can discard those. 
...
To classify the remaining 76,414 cases, we comb through the plaintiffs and classify them. We can repeat the three steps above until we have enough of them classified.
...
debt <- debt %>% 
  ...
  slice(1)
```
That gives us 26,256 lawsuits filed. 
...
In 2008 the percent with medical debt is only 4.45%. In 2017 the percent with medical debt is 0%. All other years have almost 100%. 
```{r}
...
There seems to be a steady downward trend since the peak in 2006. St. Francis medical provider seems to have increased their lawsuits. There seems to be an upward trend in the amount of medical debt. 
