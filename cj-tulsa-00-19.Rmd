...
``` {r}
debt <- debt %>% 
  mutate(plaint_type = case_when(str_detect(iss_plaint, "HEALTH|CLINIC|MEDICAL|HOSPITAL|AHS|DENTAL|ORTHOPAEDIC|ORTHOPEDIC|HILLCREST\\sRIVERSIDE\\sINC|ONCOLOGY|ST\\sJOHN\\sSAPULPA|HOME\\sIV\\sCARE\\sINC|B\\sP\\sMD|NEONATOLOGY\\sGROUP") ~ "MEDICAL",
                                 str_detect(iss_plaint, "BANK|LAWN|BOND|CASH|CREDIT|MIDLAND|TAX\\sCOMMISSION|FIA\\sCARD\\sSERVICES|CACH|TULSA\\sADJUSTMENT\\sBUREAU|TAX|CACH|ASSET\\sACCEPTANCE|UNIFUND\\sCCR\\sPARTNERS|JEFFERSON CAPITAL\\sSYSTEMS|CAVALRY|AUTO|CITY\\sOF\\sTULSA|STUDENT\\sLOAN") ~ "OTHER",
                                 TRUE ~ "UNKNOWN"))


#MEGA LIFE AND HEALTH INSURANCE COMPANY|MERITS HEALTH PRODUCTS INC|APEX MEDICAL GAS SYSTEMS INC|MAXWELL HEALTHCARE CONSULTING PLLC|TEJ HOSPITALITY INC|AMERICAN INSTITUTE OF MEDICAL TECHNOLOGY LLC|TOTAL MEDICAL PERSONNEL STAFFING LLC|TRI ANIM HEALTH SERVICES INC
```
