...
``` {r}
debt <- debt %>% 
  mutate(plaint_type = case_when(str_detect(iss_plaint, "HEALTH|CLINIC|MEDICAL|HOSPITAL|AHS|DENTAL|ORTHOPAEDIC|ORTHOPEDIC|HILLCREST\\sRIVERSIDE\\sINC|ONCOLOGY|ST\\sJOHN\\sSAPULPA") ~ "MEDICAL",
                                 str_detect(iss_plaint, "BANK|LAWN|BOND|CASH|CREDIT|MIDLAND|TAX\\sCOMMISSION|FIA\\sCARD\\sSERVICES|CACH|ASSET\\sACCEPTANCE|TULSA\\sADJUSTMENT\\sBUREAU|MIDLAND|TAX|FIA\\sCARD\\sSERVICES|CACH|ASSET\\sACCEPTANCE|UNIFUND\\sCCR\\sPARTNERS|JEFFERSON CAPITAL\\sSYSTEMS|CAVALRY|AUTO|CITY\\sOF\\sTULSA|STUDENT\\sLOAN") ~ "OTHER",
                                 TRUE ~ "UNKNOWN"))
```
...
debt <- debt %>% 
  ...
  slice(1)
```
That gives us 1,880 lawsuits filed. 
