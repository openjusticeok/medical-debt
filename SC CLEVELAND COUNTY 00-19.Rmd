...
```{r}
oscn_scrape_remaining("CLEVELAND", "SC", 2008)
oscn_scrape_remaining("CLEVELAND", "SC", 2017)
...
```{r}
mins <- ojo_query_mins("CLEVELAND", "SC", 2019, min_code = "A/")
mins <- mins %>% 
...
sc %>% 
  ...
  arrange(desc(n)) 
```
We see that over 64,704 cases are evictions. We can discard those. 
...
``` {r}
debt <- debt %>% 
  mutate(plaint_type = case_when(str_detect(iss_plaint, "HEALTH|CLINIC|MEDICAL|HOSPITAL|NEUROSURGICAL| MD|ANESTHESIA|ORTHOPAEDIC| CRNA|ENDOSCOPY| DDS| DO|GI\\sOF\\sNORMAN|MRI|NORMAN\\sREGIONAL\\sHOPITAL\\sAUTHORITY|PEDIATRICS|AMBULATORY|RADIOLOGY|ORTHOPEDIC |FOOT\\s&\\sANKLE|NEUROSURGERY|SMILE\\sSOLUTIONS|CRANIOFACIAL\\sPAIN\\sASSOCIATES|UROLOGY|SURGICAL|INTEGRIS|ORTHOPEADIC") ~ "MEDICAL",
                                 
                               
                                 str_detect(iss_plaint, "BANK|FCU|CREDIT|LOAN|FINANCIAL|LOANS|CITIFINANCIAL|FINAN|ELECTRIC|CASH|CHECKMATE|SYSCO|REALTY|SAND|BONDS|PROPERTIES|WORLD\\sACCEPTANCE\\sCORP|CROSSROADS\\sKIDZ\\sCLUB|INVESTMENT|PROPERTY|UNIVERSTY\\sOF\\sOKLAHOMA|UNIVERSITY\\sOF\\sOKLAHOMA|AUTO|GREENBRIAR\\sKINGSPARK\\sHOA|BRSI|JEEP|MOTOR|OKLAHOMA\\sSTATE\\sOF\\sEX|HOMEOWNERS|CHIROPRACTIC") ~ "OTHER",
                                 TRUE ~ "UNKNOWN"))
```
...
There is no clear trend, further data query is necessary. 
