---
title: "Small Claims: Cleveland County 00-19"
author:
...
This analysis focuses on small claims court cases in Cleveland County.
...
sc <- ojo_query_disps("CLEVELAND", "SC", 2000:2019) 



```
This gives us over 137,239 rows of data. Each row in the table is a disposition; since there may be multiple dispositions per case, we should expect that the number of cases is somewhat lower.
...
```{r}
mins <- ojo_query_mins("CLEVELAND", "SC", 2000:2019, min_code = "A/")
mins <- mins %>% 
...
There are 108,695 small claims cases. 
# Defining Measures
...
sc %>% 
  ...
  arrange(desc(n)) 
```
We see that over 92,836 cases are evictions. We can discard those. 
...
To classify the remaining 873,44 cases, we comb through the plaintiffs and classify them. We can repeat the three steps above until we have enough of them classified.
...
"HEALTH|CLINIC|MEDICAL|HOSPITAL| MD|ANESTHESIA| CRNA|ENDOSCOPY| DDS| DO|MRI|NORMAN\\sREGIONAL\\sHOPITAL\\sAUTHORITY|PEDIATRICS|AMBULATORY|RADIOLOGY|SMILE\\sSOLUTIONS|SURGICAL|INTEGRIS|NEUROSURGICAL|ORTHOPAEDIC|GI\\sOF\\sNORMAN|ORTHOPEDIC|NEUROSURGERY|PAIN\\sASSOCIATES|UROLOGY|ORTHOPEADIC|CHIROPRACTIC|ANKLE\\sSPECIALISTS") ~ "MEDICAL",
                                 str_detect(iss_plaint, "BANK|FCU|CREDIT|LOAN|FINANCIAL|LOANS|CITIFINANCIAL|FINAN|ELECTRIC|CASH|CHECKMATE|SYSCO|REALTY|SAND|BONDS|PROPERTIES|WORLD\\sACCEPTANCE\\sCORP|CROSSROADS\\sKIDZ\\sCLUB|INVESTMENT|PROPERTY|UNIVERSTY\\sOF\\sOKLAHOMA|AUTO|GREENBRIAR\\sKINGSPARK\\sHOA|BRSI|JEEP|MOTOR|OKLAHOMA\\sSTATE\\sOF\\sEX|HOMEOWNERS") ~ "OTHER",
                                 TRUE ~ "UNKNOWN"))
...
```
That gives us 9,743 lawsuits filed. 
...
ggplot(year_sum, aes(file_year, n)) +
  ...
  labs(title = "Small claims medical debt cases",
       subtitle = "Cleveland County 2000-2019")
```

##summarizing by provider 
The most common plaintiffs seem to be NORMAN REGIONAL, INTEGRIS, NORMAN ANESTHESIA PROVIDERS. Again, we’ll use the str_detect() function to classify the cases. 

``` {r}
debt <- debt %>% 
  mutate(plaint_cat = case_when(str_detect(iss_plaint, "NORMAN REGIONAL") ~ "NORMAN REGIONAL HEALTH",
                                str_detect(iss_plaint, "INTEGRIS") ~ "INTEGRIS",
                                str_detect(iss_plaint, "NORMAN ANESTHESIA PROVIDERS") ~ "NORMAN ANESTHESIA PROVIDERS",
                                TRUE ~ "OTHER"))
...
There was a clear upward trend followed by a decline in number of medcial debt cases. 
