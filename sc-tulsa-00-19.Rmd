...
This gives us over 569,053 rows of data. Each row in the table is a disposition; since there may be multiple dispositions per case, we should expect that the number of cases is somewhat lower.
...
sc %>% 
  ...
  arrange(desc(n)) 
```
We see that over 354,891 cases are evictions. We can discard those. 
...
To classify the remaining 119,168 cases, we comb through the plaintiffs and classify them. We can repeat the three steps above until we have enough of them classified.
...
``` {r}
debt <- debt %>% 
  mutate(plaint_type = case_when(str_detect(iss_plaint, "HEALTH|CLINIC|MEDICAL|HOSPITAL|MED|AHS|OKLAHOMA\\sHEART\\sINC|PHYSICIANS| MD| DDS|D\\.O\\.| DO|ANESTHESIA|ENDOSCOPY|NEUROSURGERY|NEONATOLOGY|ANESTHESIOLOGIST|NEUROLOGICAL|RESPIRATORY|NEUROSPINE|ORTHOPAEDIC|GASTROENTEROLOGY|FOOT\\sAND\\sANKLE|NOSE|UROLOGIC|ORTHOPEDIC|THERAPY\\sSPECIALISTS|ST\\sJOHN\\sSAPULPA|ANESTEHSIOLOGIST|HILLCREST\\sRIVERSIDE\\sINC\\sDBA|TULSA\\sHOME\\sTHERAPY|ANESTHEISOLOGIST|ORTHOPEDIC|GALLERIA\\sOF\\sSMILES|CHERRY\\sSTREET\\sMRI\\sINC\\sDBA|ORTHOTICS") ~ "MEDICAL",
...
debt <- debt %>% 
  ...
  slice(1)
```
That gives us 32,139 lawsuits filed. 
...
>>>>>>> 886850b0ddaa0b6e6aa37d8eda2789c766409739
...
```{r}
new <- new %>% 
  mutate(debt_amt_na = case_when(debt_amt == "" | is.na(debt_amt) ~ TRUE,
                                 TRUE ~ FALSE))
...
There seems to be a steady downward trend in medical debt cases since the peak in 2006. 
